...
public class InstallerManager {
	...
	private void install(String application) {
		String installerPath = settings.getNetworkPath(application);
		String arguments = settings.getArguments(application);

		try {
			Process installer = new ProcessBuilder(installerPath, arguments)
					.start();
			installer.waitFor();
	...
	public void startInstallation(Map<String, Boolean> softwareToInstall) {
		for (Entry<String, Boolean> application : softwareToInstall
				.entrySet()) {
